...
public class Sphere extends Figure {
    ...
    private double intersectGeometric(Ray ray) {
        ...
        double tCA = MathUtils.dotProduct(vectorDifference, vectorDirection);

        if (tCA < 0) {
            return Double.POSITIVE_INFINITY;
        }

        double LSquare = MathUtils.dotProduct(vectorDifference, vectorDifference);
        ...
        if (MathUtils.dotProduct(vectorDifference, vectorDifference) < LSquare) {
